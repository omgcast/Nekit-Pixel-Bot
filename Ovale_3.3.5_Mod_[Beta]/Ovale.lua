...
local KEY_MAP = {}
for i, v in ipairs(KEYS_LIST) do KEY_MAP[v] = i end

local MOD_MAP = { ["S"]=1, ["C"]=2, ["A"]=3, ["SC"]=4, ["SA"]=5, ["CA"]=6 }

local function FastSearch(frame)
    ...
    for _, region in ipairs(regions) do
        if region:GetObjectType() == "FontString" then
            ...
            if t and t ~= "" then
                t = t:gsub("%s+", ""):upper()
                local keyOnly = t:match("%-(.+)") or t
                if KEY_MAP[keyOnly] and not t:find(":") and not t:find("%.") then
                    return t
                end
            end
        end
...
Bridge:SetScript("OnUpdate", function(self, elapsed)
    self.tex:SetVertexColor(1, 0, 1, 1)

    if not UnitExists("target") or UnitIsDead("target") or UnitIsUnit("player", "target") then
        return
    end

    local text = nil
    for i=1, 4 do
        local icon = _G["OvaleIcone_"..i] or _G["OvaleIcone"..i]
        if icon then text = FastSearch(icon) end
        if text then break end
    end
    
    if not text and Ovale.frame then
        text = FastSearch(Ovale.frame.frame or Ovale.frame)
    end
    ...
    if kId and kId > 0 then
        self.tex:SetVertexColor(kId/255, mId/255, 1, 1)
    end
end)